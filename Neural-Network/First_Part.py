...
x1 = np.random.uniform(-150, 150, size=number_of_points)
y1 = random1 * x1 + random2
y2 = np.abs(x1) * (1 / 10)
y3 = np.power(x1, 3) + 1
y4 = np.sin(x1) + np.cos(x1)
...
clf1 = MLPRegressor(alpha=0.1, hidden_layer_sizes=(20, 20, 20), max_iter=1000,
                    activation='logistic', verbose='True', learning_rate='adaptive')
clf2 = MLPRegressor(alpha=0.1, hidden_layer_sizes=(20, 20, 20), max_iter=1000,
                    activation='relu', verbose='True', learning_rate='adaptive')
clf3 = MLPRegressor(alpha=0.1, hidden_layer_sizes=(20, 20, 20), max_iter=1000,
                    activation='tanh', verbose='Trues', learning_rate='adaptive')
clf4 = MLPRegressor(alpha=0.1, hidden_layer_sizes=(20, 20, 20), max_iter=1000,
